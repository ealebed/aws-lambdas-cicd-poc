...
- This repository is personal (`ealebed/aws-lambdas-cicd-poc`), so organization ruleset bypass lists are not available.
...
- PR Validation only runs language jobs when a Lambda directory changed. Actions-only and shared-Dockerfile PRs skip `validate-*`. Requiring those skipped names (`Validate Python Lambda - ${{ matrix.lambda }}`) blocks merge. Requiring only `Detect Changed Lambdas` lets a failing Go/Python/JS validation pass auto-merge.
...
Add a `PR checks` job that always runs after detect + validate jobs. It succeeds when detect succeeded and every validate job is `success` or `skipped`. Require that check (not the skipped matrix names, not Lambdas Release).

Keep `.github/CODEOWNERS` so humans still get review requests. Do **not** enable “Require review from Code Owners”. Require **one** approving review plus `PR checks`. The auto-merge workflow runs only when the PR author is `dependabot[bot]`.
...
Same merge pattern as `ealebed/token-injector`.
...
### Require only `Detect Changed Lambdas`
- Pros: That job already always reports.
- Cons: Go module Dependabot PRs could squash-merge while `Validate Go Lambda` is red.
- Rejected: `PR checks` aggregates detect + validations.

### Always run all language validations
- Pros: No skipped jobs.
- Cons: Every Actions bump would build/test all six example Lambdas.
- Rejected: Keep path filters; aggregate with `PR checks`.
...
- Shared Dockerfile PRs do not run per-Lambda tests; `PR checks` still passes if detect succeeded.
