...
| `ecr-copy-image-to-region.sh` | Output image URI for a region: use primary URI if same region, else pull from primary ECR and push to current region. Retries up to 3 times with backoff; verifies image exists in target region after push. Output: `uri`. |
