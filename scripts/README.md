...
| `ecr-copy-image-to-region.sh` | Output image URI for a region: use primary URI if same region, else pull from primary ECR and push to current region. Retries copy (3x, backoff) on failure. No post-push verification (ECR describe-images is unreliable in CI). Output: `uri`. |
